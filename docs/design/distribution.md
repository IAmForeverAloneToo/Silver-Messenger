...
| Installer | `deploy/install.sh` is in the repository and read before it is run; from 0.12.2 it is not a release asset, because a release page carries the program and nothing else. What replaced it as the checkable path is better: the relay is one file on the page, covered by a `SHA256SUMS` the project signs, so an operator verifies a signature and a checksum rather than reading a script (README, "Deploying a relay"). |
