...
| What ships next | The remaining Mediums and the Lows as 0.11.0, the report's section 13.2 as verified, each with its own tests; the three that need a wire change (the sequencer tombstone token, the leaf-per-identity rule, the parked-size caps) go in as optional fields and rules that honest 0.10.x peers never trip. |
...
| How the relay is protected meanwhile | The test relay runs the fixed relay from the day the relay fixes land on `main`, through the deploy workflow, before the release is cut. Other operators upgrade with the release; the changelog and the advisory say why. |
| Disclosure | The findings are the maintainer's to publish. The report goes into the repository whole, with the patch release; one GitHub Security Advisory per Critical and High finding, credited to the reviewers as the report names them; the changelog's `Security` section lists them. |
...
| SM-R-07 | Medium | C. | The mailbox paged into the outbound queue as acknowledgements arrive; a write timeout; blob chunks streamed. | 0.11.0 |
| SM-R-08 | Medium | P: the race before the creator's first `group_create` is not real (the id is random and registered first) and the epoch disclosure is specified; the 180-day expiry and a restore from an older backup are the real openings. | Expired entries kept as tombstones; re-creation of a known id needs a token of the last epoch (an optional field, so a 0.10.x client is told `forbidden` and marks the group out of sync until it upgrades); a restore from before a group's creation reopens the id, and the operator's guide says so. | 0.11.0 |
...
| SM-C-05 | Medium | C. | The features a host offered are remembered in the config; a feature that goes missing is a downgrade event, with `transparency` kept on so the lookups are refused until `/relay accept`. | 0.11.0 |
...
| SM-C-07 | Medium | P: replies are not readable by a holder of the identity key alone (the outer layer is sealed to the pinned X25519 key); the impact is unflagged impersonation towards the recipient and lost replies for the real contact. | The inbound handshake's `identity_dh` reported with the session and compared with the pin; a mismatch warns like a key change and drops the session. | 0.11.0 |
...
| SM-C-09 | Medium | C. | `--check-release` reads the config's proxy when no proxy is given on the command line or in the environment. | 0.11.0 |
| SM-C-10 | Medium | C. | The data directory 0700, every store file 0600, downloads and exports likewise, the outbox and log stores synced. | 0.11.0 |
| SM-C-11 | Medium | P: the Windows, macOS and macro-document omissions are real; the Linux additions are not (no executable bit; nothing opens `.service`); the mark-of-the-web gap is Windows-only. | An allowlist of viewer types with the denylist as a second gate; the `.open/` copy tagged. | 0.11.0 |
...
| SM-C-13 | Low | C. | The event says whether the sender was verified; the notice is dropped for blocked ids, worded as a claim when unverified, and throttled. | 0.11.0 |
| SM-C-14 | Low | C. | A cap on peers with sessions, the least recently used evicted first. | 0.11.0 |
| SM-C-15 | Low | P: the replay applies to v1 bodies only; the ML-KEM private half is a 64-byte seed, so the bloat is kilobytes per reconnect. | Seen epochs remembered, a window for late sequence numbers, a cap on handed-out keys kept and on top-ups per hour. | 0.11.0 |
| SM-C-16 | Low | C. | A `Submission` event and a status-bar word; `--require-anonymous`. | 0.11.0 |
...
| SM-C-18 | Low | C. | The old head and checkpoints kept beside a fork or rewind; lookups refused until `/log accept`; the first sync worded as trust on first use. | 0.11.0 |
| SM-C-19 | Low | C. | One rewrite per deletion body; compaction of orphan updates past the window at load. | 0.11.0 |
| SM-C-20 | Low | C. | Control characters filtered on the four paths; a multi-line message one journal sentence; continuation rows marked. | 0.11.0 |
| SM-C-21 | Low | C. | Argon2 parameters bounded on read. | 0.11.0 |
| SM-C-22 | Low | C. | Passphrases in zeroising strings; the environment passphrase dropped after the first unlock unless asked to keep it. | 0.11.0 |
| SM-C-23 | Low | P: without a key store the key does rotate, through a plaintext hop on disk. | A passphrase change re-encrypts under a fresh key without a plaintext state. | 0.11.0 |
...
| SM-C-26 | Low | C. | A `note` flag in history entries; a separate keystroke for `/revoke`, `/rotate` and `/devices leave`; a paste heuristic where bracketed paste is missing. | 0.11.0 |
...
1. 0.10.1, in this order, each with its tests and its documentation:
   ...
   and this note, the release, the advisories, the deployed relay.
