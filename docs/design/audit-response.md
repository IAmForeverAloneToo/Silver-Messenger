...
| SM-P-14 | Info | C; where a body carries an `id` it wins over the envelope's, so only bodies without one are exposed to renaming. | §12.1 says which id is the authenticated one. Every body carries its id from 0.16.0, and a body without one is refused from 0.17.0, so no body is read under the envelope's. | doc, 0.16.0, 0.17.0 |
