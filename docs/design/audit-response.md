...
| What waits for 1.0 | Four of the report's section 13.3: message ids inside the authenticated body (SM-P-14), rollback protection for the key-bearing files (SM-C-24), the identity-key binding of the v4 handshake (SM-P-04), HMAC-named history files (SM-C-25). Each is a protocol or on-disk format change that deserves its own design note; the roadmap lists them under item 57. The rest of that section's items are done: the device counter-signature's root cause (SM-R-01) went out in 0.10.1, the length-prefixed ratchet AAD (SM-P-06), the per-epoch sequence window (SM-C-15), data-key rotation (SM-C-23) and continuous fuzzing (SM-S-08) in 0.11.0. |
...
| SM-G-03 | Medium | P for its third case (the admins get `MissingProposal`, nobody is blamed; the desynchronisation is real). | Done in 0.11.0. A member's own Remove may be referenced by any committer; a non-admin's commit does not consume the proposal store; a reader tolerates an admin whose every leaf left by its own proposal; and a handshake at or above this client's epoch that will not stage marks the group out of sync rather than leaving it stuck. | 0.11.0 |
| SM-G-04 | Medium | C; Update proposals are unreachable today, so the update-path leaf is the whole of it. | Done in 0.11.0: the committer's new leaf and any an Update proposal carries verified as an added leaf is, and required to keep their account and device; a tree that cannot be read after the merge marks the group broken by that commit's committer instead of surfacing at the next one. | 0.11.0 |
| SM-G-05 | Medium | C; OpenMLS exposes no way to verify the other leaves before `into_group`. | Done in 0.11.0: at most twenty invitations wait at once; any failure after `into_group` deletes the group's storage; the credential check that is possible is done before. | 0.11.0 |
| SM-G-06 | Medium | C, at the low end: the amplifier needs a contact. | Done in 0.11.0. A group where this account is an admin only by its Welcome's word does an admin's automatic work on the user's ask alone; rejoins are limited to four per member per group per hour. Not confirmations, which would break the documented link-join flow for every appointed admin. | 0.11.0 |
| SM-G-07 | Medium | C. | Done in 0.11.0: the new device shows the account it is about to belong to and asks before adopting the link, `--link --account <id>` answers in advance, and a refusal leaves the link standing. The AAD is not changed (it would break 0.10.0 primaries). | 0.11.0 |
| SM-G-08 | Medium | C. | Done in 0.11.0: at most nine leaves per identity and at most 256 × 9 in all, refused by readers and by `stage_add`. | 0.11.0 |
| SM-G-09 | Low | C. | Done in 0.11.0: `not_found` re-creates the entry at the group's own epoch and `stale` behind the group replays the tokens kept, both from the terminal client. | 0.11.0 |
| SM-G-10 | Low | C. | Done in 0.11.0: the Welcome is parsed before any state is given up. A well-formed Welcome that then fails a check still costs the old state, because OpenMLS claims the group id as the staged Welcome is read. | 0.11.0 |
| SM-G-11 | Low | C. | Done in 0.11.0: the list is trimmed to what a deposit may hold before every deposit, oldest first, their secrets with them. | 0.11.0 |
| SM-G-12 | Low | C. | Done in 0.11.0: everything received is persisted on the way out, success or failure. | 0.11.0 |
| SM-G-13 | Info | C, all four; the `Head` bullet was traced to the fork alarm. | Done in 0.11.0. The sender ratchet set to the documented 64 on both configs; a gossiped head taken only from a group the user is in and a sender not blocked; the invite link's duplicate check per identity as §13.7 always said, and at most 32 asks answered per invite key; a bundle pinned or a verified mark set over `sync contact` remembered against the device that sent it and undone when that device is unlinked. | 0.11.0 |

### 3.5 Deployment, packaging, CI and supply chain

| ID | Sev. | Verdict | Decision | Release |
| --- | --- | --- | --- | --- |
| SM-S-01 | Medium | C; the unhedged sentences are the README's and SECURITY.md's, not the threat model's. | Done in 0.11.0. The workflow's signing step goes: a key it could use lives where the build lives and says what the attestation says. Signing is documented as an out-of-band step on a maintainer machine, and until a key exists the README, SECURITY.md, the assessment (1.14.2 down to Partly), the threat model and the distribution note all say plainly that releases carry the provenance attestation and no maintainer signature. | 0.11.0 |
| SM-S-02 | Low | C; the release was built with 1.98.1, not the 1.94.1 the report suggests. | Done in 0.11.0: `rust-toolchain.toml` pins 1.98.1, a local composite action installs what the file says in every job (the workflows asked for "stable", which overrode it), the Dockerfile's Rust image follows, and each release carries a `BUILD-INFO.txt` per target naming the compiler, the commit and the flags. | 0.11.0 |
| SM-S-03 | Low | C; the README documents the plaintext outcome outright rather than treating `wss://` as the default. | Done in 0.11.0: the installer published as a release asset under `SHA256SUMS` and the README telling operators to check it; `rustup-init` fetched from the Rust project's host and checked against its published hash; a non-loopback plaintext listener refused without `SILVER_ALLOW_PLAINTEXT=1`; the ipify lookup dropped for the machine's own address (`SILVER_PUBLIC_IP` behind NAT); the token-in-URL risk documented; the `sed` replaced with awk. | 0.11.0 |
| SM-S-04 | Low | C. | Done in 0.11.0: the host key comes from the repository variable `VPS_HOST_KEY`, `StrictHostKeyChecking=yes`, and a deploy without it stops and says where to get the key. | 0.11.0 |
| SM-S-05 | Low | C, plus the BuildKit frontend. | Done in 0.11.0: the Rust and Alpine bases, the BuildKit frontend, and the Debian and Arch containers CI runs the package checks in are all pinned by `@sha256:`. | 0.11.0 |
| SM-S-06 | Info | C; the admin-socket remark is overstated (the runtime directory is 0700 under systemd). | Done in 0.11.0: the directives added; and the relay shuts its socket's directory to 0700 before binding, which closes the `bind`-to-`chmod` window wherever it runs rather than only under the unit. | 0.11.0 |
| SM-S-07 | Info | C. | Done in 0.11.0: `multiple-versions` set to deny with the 33 current duplicates skipped by name, so a duplicate this repository introduces fails while upstream's move freely; `cargo audit` run over the fuzz lockfile too; what the tree carries recorded in the threat model. | 0.11.0 |
| SM-S-08 | Info | C; a corpus cannot live in a branch under this repository's rules. | Done in 0.11.0: each target's corpus cached per target and restored on every run, a weekly schedule giving each half an hour, and an `identifier` target over the base58 and JSON id path at the frame size the relay accepts. | 0.11.0 |
| SM-S-09 | Info | C; the pull-request exposure is theoretical here. | Done in 0.11.0: the release job named into a `release` environment, where a protection rule can be attached; GitHub's generated notes turned off, so the notes are the changelog section alone; `packaging/update.sh` verifying `SHA256SUMS` against `minisign.pub` when the release is signed. The Homebrew job installing from the live release stays: it tests what a person gets. | 0.11.0 |
...
| 1.0 | The four the report's section 13.3 leaves, each of which changes a wire or an on-disk format and each with a design note first: SM-P-14, SM-C-24, SM-P-04, SM-C-25. Roadmap item 57 | Planned |
